...
# Адаптивный режим картинки: тип определяется и упражнение выполняется ОДНИМ
# vision-вызовом, без отдельного классификатора. Правила времени/структуры живут
# внутри веток (для типа 2 прошедшее время и nejdřív/nakonec — неправильный жанр).
_IMAGE_TASK = """Сначала определи, что на изображении, и выполни соответствующее задание.

Тип 1 — обычная фотография, одна сцена. Составь по ней историю из 6–8 предложений
в прошедшем времени (minulý čas). Обязательная структура повествования: начни с
«Nejdřív…», развивай через «Potom…» и «Pak…», закончи «Nakonec…».

Тип 2 — учебный лист (pracovní list): несколько пронумерованных картинок,
заголовок-тема, возможно блок опорных фраз-конструкций (например «Jak popisovat
obrázek?»). Это упражнение popis obrázku — выполни его так, как оно задумано:
- Первой строкой — заголовок: 📚 и тема листа.
- Опиши КАЖДУЮ пронумерованную картинку по 2–3 предложения в НАСТОЯЩЕМ времени,
  нумеруя как на листе (1️⃣, 2️⃣, …).
- Обязательно используй опорные конструкции с самого листа (если блок фраз есть)
  и лексику темы листа: для темы «roční období a počasí» в каждом описании назови
  сезон и погоду; конструкцию «Žena má na sobě…» — там, где на картинке видна
  одежда, и т.п.
- Структуру nejdřív/potom/pak/nakonec здесь НЕ используй.
- Если на листе есть блок опорных фраз — после Slovníčku выведи отдельный блок
  «📌 Opěrné fráze z listu:» со списком этих фраз ДОСЛОВНО как на листе.
  Если блока опорных фраз на листе нет — этот блок не выводи.

Тип 3 — на изображении нет ни сцены, ни учебного листа (скриншот переписки,
страница сплошного текста и т.п.): коротко и честно ответь по-русски, что не
видишь ни сцены для истории, ни учебного листа. Ничего не выдумывай; Slovníček
в этом случае не нужен."""


def generate_story_from_image(image_b64: str, vocab: list = None,
                              media_type: str = "image/jpeg") -> str:
    """История по фото ИЛИ разбор учебного листа. Картинка не сохраняется/не логируется."""
    system = _story_system(_IMAGE_TASK, vocab, past_tense=False, narrative=False)
    return _complete_vision(system, "Выполни задание по этому изображению.", image_b64,
                            media_type=media_type, max_tokens=1600)
